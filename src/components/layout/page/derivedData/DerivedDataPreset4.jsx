...
export default function DerivedDataPreset4({ endpoints={} }) {
...
const [variable4Volume, setVariable4Volume] = useState(100);
  const variable4SetVolume = volume => {
    ...
    vpdGainNode.gain.linearRampToValueAtTime(rescaleVolume(volume), audioCtx.currentTime + 0.01);
  };
  ...
  useEffect(() => {
    ...
    else vpdMuteNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume), audioCtx.currentTime + 0.01);
  }, [variable4Mute]);
...
function createFluteLikeVoice(freq) {
  ...
  osc.connect(tremoloGain).connect(baseGain).connect(humidityGainNode);
...
function createSoftPercussiveVoice() {
  const audioCtx = audioCtxRef.current;
  const vpdGainNode = vpdGainNodeRef.current;
  const gain = audioCtx.createGain();
...
function playVoices(record, activeInstruments) {
  ...
  // Stop old percussion voice
  if (currentPercussionVoice?.stop) {
    currentPercussionVoice.stop();
  }

  const voice = instruments[variable]();
...
setInterval(() => {
  ...
  const playSound = () => {
    if (!play) {
      ...
      vpdGainNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume), audioCtx.currentTime + 0.01);
  ...
  const variable4HideOnClick = () => {
    ...
    if (!dials) graphRef.current.toggleVariableVisibility('vpd', variable4Hide);
  };
...
const dialVariables = [
  ...
  return (
    <PageLayout title='Derived Data Preset 4' backLink='derived-data' help helpOnClick={helpClick} dataControls treeOnClick={treeClick} aboutOnClick={aboutClick}>
      {
          loading
            ? <h4>Loading...</h4>
            : hide
              ? <div style={{ height: '60em', width: '100%', marginBottom: '2rem', backgroundColor: 'whitesmoke', border: '1px solid lightgray' }}>
              ...
              : initialiseVisuals
                ...
                : <div style={{ height: '60em', width: `${dials ? '100' : remap(graphZoom)}%`, marginBottom: '2rem' }}><Dials ref={graphRef} variables={dialVariables} data={incrementalData} highlightedIndex={highlightedIndex} loading={loading} /></div>
      }
