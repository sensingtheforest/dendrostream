...
import birdsong from '../../../../assets/birdsong.wav';
import crickets from '../../../../assets/crickets.wav';

export default function DerivedDataPreset5({ endpoints={} }) {
  ...
  const ambientSoundsGainNodeRef = useRef(null);
  ...
  const ambientSoundsMuteNodeRef = useRef(null);
  ...
  const ambientSoundsPannerNodeRef = useRef(null);
  ...
  const variable4SetVolume = volume => {
    ...
    vpdGainNode.gain.linearRampToValueAtTime(rescaleVolume(volume * 1.5), audioCtx.currentTime + 0.01);
  };
  ...
  useEffect(() => {
    ...
    else vpdMuteNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume * 1.5), audioCtx.currentTime + 0.01);
  }, [variable4Mute]);
...
function playMelody(value, min, max) {
  ...
  createSynth(freq, 'triangle', 1, 2.5, displacementGainNode);
}
...
function setScaleFromVPD(vpd) {
  scaleType = (vpd >= 0.8 && vpd <= 2.2) ? 'major' : 'minor';
}
...
const ambientIsPlayingRef = useRef(false);

async function playAmbientSound(soundUrl) {
  if (ambientIsPlayingRef.current) return;
  const audioCtx = audioCtxRef.current;
  const ambientSoundsGainNode = ambientSoundsGainNodeRef.current;

  try {
    const response = await fetch(soundUrl);
    const arrayBuffer = await response.arrayBuffer();
    const ambientBuffer = await audioCtx.decodeAudioData(arrayBuffer);
    const bufferSource = audioCtx.createBufferSource();
    bufferSource.buffer = ambientBuffer;
    bufferSource.loop = true;
    bufferSource.connect(ambientSoundsGainNode);
    bufferSource.start();

    ambientSourcesRef.current.push(bufferSource);
    ambientIsPlayingRef.current = true;
  } catch (e) {
...
function stopAllAmbientSounds() {
  ...
  ambientIsPlayingRef.current = false;
}
...
const playSound = () => {
    if (!play) {
      ...
      const ambientSoundsGainNode = audioCtx.createGain();
      ambientSoundsGainNodeRef.current = ambientSoundsGainNode;
      ...
      const ambientSoundsMuteNode = audioCtx.createGain();
      ambientSoundsMuteNodeRef.current = ambientSoundsMuteNode;
      ...
      vpdPannerNode.setPosition(0, 0, 0); // Centre
      ...
      const ambientSoundsPannerNode = audioCtx.createPanner();
      ambientSoundsPannerNodeRef.current = ambientSoundsPannerNode;
      ambientSoundsPannerNode.panningModel = 'HRTF';
      ambientSoundsPannerNode.distanceModel = 'inverse';
      ambientSoundsPannerNode.setPosition(0, 0, 0); // Centre
      ...
      vpdGainNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume * 1.5), audioCtx.currentTime + 0.01);
      drumsGainNode.gain.linearRampToValueAtTime(rescaleMasterVolume(volume * 0.5), audioCtx.currentTime + 0.01);
      ambientSoundsGainNode.gain.linearRampToValueAtTime(rescaleMasterVolume(volume * 0.5), audioCtx.currentTime + 0.01);
      ...
      ambientSoundsGainNode.connect(ambientSoundsMuteNode);
      ...
      ambientSoundsMuteNode.connect(ambientSoundsPannerNode);
      ...
      ambientSoundsPannerNode.connect(masterVolumePannerNode);
  ...
  const run = async () => {
    if (play && incrementalData.length > 0) {
      ...
      setScaleFromVPD(record.vpd);
      playChord(record.temperature, -20, 60);
      playMelody(record.displacement, 0, 1);
      playGuitarNote(record.humidity, 0, 100, record.vpd < 0.8 || record.vpd > 2.2);
      playBassNote(record.soilMoisture, 0, 100, record.soilMoisture);
      playDrums(remapReversed(playbackSpeed));

      const inRange = record.vpd > 0.8 && record.vpd < 2.2;

      if (inRange) {
        // Only trigger if not already playing
        if (!ambientIsPlayingRef.current) {
          await playAmbientSound(birdsong);
          await playAmbientSound(crickets);
        }
      } else {
        // If playing and out of range, stop sounds
        if (ambientIsPlayingRef.current) {
          stopAllAmbientSounds();
        }
      }

      // Stop ambient if this is the last data point
      if (index === incrementalData.length - 1 && !inRange) {
        stopAllAmbientSounds();
      }
    }
  };
...
const sceneVariables = [
  ...
  return (
    <PageLayout title='Derived Data Preset 5' backLink='derived-data' help helpOnClick={helpClick} dataControls treeOnClick={treeClick} aboutOnClick={aboutClick}>
      {
          loading
            ? <h4>Loading...</h4>
            : hide
