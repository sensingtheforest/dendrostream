...
export default function DerivedDataPreset1({ endpoints={} }) {
...
const [variable4Volume, setVariable4Volume] = useState(100);
  ...
  const playSound = () => {
    if (!play) {
      ...
      displacementOscillator.connect(displacementGainNode);
      soilMoistureOscillator.connect(soilMoistureGainNode);
      temperatureOscillator.connect(temperatureGainNode);
      humidityOscillator.connect(humidityGainNode);
      vpdOscillator.connect(vpdGainNode);
      ...
      displacementOscillator.type = 'sine';
      soilMoistureOscillator.type = 'triangle';
      temperatureOscillator.type = 'sine';
      humidityOscillator.type = 'triangle';
      vpdOscillator.type = 'sawtooth';
...
useEffect(() => {
  ...
  return (
    <PageLayout title='Derived Data Preset 1' backLink='derived-data' help helpOnClick={helpClick} dataControls treeOnClick={treeClick} aboutOnClick={aboutClick}>
      {
          loading
            ? <h4>Loading...</h4>
            : hide
              ...
              : initialiseVisuals
                ...
                : <div style={{ height: '40em', width: `${remap(graphZoom)}%`, marginBottom: '2rem' }}><SingleLineGraph ref={graphRef} data={incrementalData} highlightedIndex={highlightedIndex} zoom={remap(graphZoom)} loading={loading} /></div>
      }
