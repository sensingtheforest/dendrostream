...
export default function DerivedDataPreset3({ endpoints={} }) {
  ...
  const treeMeanGrowthVolumeGainNodeRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (variable4Mute) treeMeanGrowthMuteNode.gain.linearRampToValueAtTime(0.0, audioCtx.currentTime + 0.01);
    else treeMeanGrowthMuteNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume * 0.75), audioCtx.currentTime + 0.01);
  }, [variable4Mute]);
  ...
  const playSound = () => {
    if (!play) {
      ...
      const treeMeanGrowthVolumeGainNode = audioCtx.createGain();
      treeMeanGrowthVolumeGainNodeRef.current = treeMeanGrowthVolumeGainNode;
      ...
      treeMeanGrowthPannerNode.setPosition(0, 0, 0); // Centre
      ...
      treeMeanGrowthGainNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume * 0.75), audioCtx.currentTime + 0.01);
      treeMeanGrowthVolumeGainNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume * 0.75), audioCtx.currentTime + 0.01);
      ...
      treeMeanGrowthSynth.connect(treeMeanGrowthGainNode);
      ...
      treeMeanGrowthPannerNode.connect(temperatureStereoPannerNode);

      temperatureStereoPannerNode.connect(masterVolumePannerNode);
      referenceNotePannerNode.connect(masterVolumePannerNode);

      masterVolumePannerNode.connect(treeMeanGrowthVolumeGainNode);
      treeMeanGrowthVolumeGainNode.connect(masterVolumeGainNode);
  ...
  useEffect(() => {
    if (play && incrementalData.length > 0) {
      ...
      const treeMeanGrowthVolumeGainNode = treeMeanGrowthVolumeGainNodeRef.current;
      ...
      treeMeanGrowthVolumeGainNode.gain.linearRampToValueAtTime(rescaleFrequency(0.0, 0.2, 0.4, 1, record?.treeMeanGrowth), audioCtx.currentTime + 0.01);
  ...
  const variable4HideOnClick = () => {
    ...
    graphRef.current.toggleVariableVisibility('treeMeanGrowth', variable4Hide);
  };
...
useEffect(() => {
  ...
  return (
    <PageLayout title='Derived Data Preset 3' backLink='derived-data' help helpOnClick={helpClick} dataControls treeOnClick={treeClick} aboutOnClick={aboutClick}>
      {
          loading
            ? <h4>Loading...</h4>
            : hide
      ...
      <PresetOptionsPanel
        endpoints={endpoints}
        dataRange={dataPeriod}
        dataRangeSet={dataPeriodSet}
        ...
        variable4Name='Tree Mean Growth'
