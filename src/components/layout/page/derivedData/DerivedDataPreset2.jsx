...
export default function DerivedDataPreset2({ endpoints={} }) {
  ...
  const vpd1OscillatorRef = useRef(null);
  ...
  const vpd1GainNodeRef = useRef(null);
  ...
  const vpd1MuteNodeRef = useRef(null);
  ...
  const vpd1PannerNodeRef = useRef(null);
  ...
  const variable4SetVolume = volume => {
    ...
    const audioCtx = audioCtxRef.current;
    const vpd1GainNode = vpd1GainNodeRef.current;
    const vpd2GainNode = vpd2GainNodeRef.current;
    vpd1GainNode.gain.linearRampToValueAtTime(rescaleVolume(volume * 0.75), audioCtx.currentTime + 0.01);
  ...
  useEffect(() => {
    if (isFirstRender.current) return; 
    const audioCtx = audioCtxRef.current;
    const vpd1MuteNode = vpd1MuteNodeRef.current;
    const vpd2MuteNode = vpd2MuteNodeRef.current;
    if (variable4Mute) {
      vpd1MuteNode.gain.linearRampToValueAtTime(0.0, audioCtx.currentTime + 0.01);
      vpd2MuteNode.gain.linearRampToValueAtTime(0.0, audioCtx.currentTime + 0.01);
    }
    else {
      vpd1MuteNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume * 0.75), audioCtx.currentTime + 0.01);
  ...
  const playSound = () => {
    if (!play) {
      ...
      const vpd1Oscillator = audioCtx.createOscillator();
      vpd1OscillatorRef.current = vpd1Oscillator;
      ...
      const vpd1GainNode = audioCtx.createGain();
      vpd1GainNodeRef.current = vpd1GainNode;
      ...
      const vpd1MuteNode = audioCtx.createGain();
      vpd1MuteNodeRef.current = vpd1MuteNode;
      ...
      const vpd1PannerNode = audioCtx.createPanner();
      vpd1PannerNodeRef.current = vpd1PannerNode;
      vpd1PannerNode.panningModel = 'HRTF';
      vpd1PannerNode.distanceModel = 'inverse';
      vpd1PannerNode.setPosition(0, 0, 0); // Centre
      ...
      vpd2PannerNode.setPosition(0, 0, 0); // Centre
      ...
      vpd1GainNode.gain.linearRampToValueAtTime(rescaleVolume(variable4Volume * 0.75), audioCtx.currentTime + 0.01);
      ...
      vpd1Oscillator.connect(vpd1GainNode);
      ...
      vpd2Oscillator.connect(vpd2GainNode);
      ...
      vpd1GainNode.connect(vpd1MuteNode);
      ...
      vpd1MuteNode.connect(vpd1PannerNode);
      ...
      vpd1PannerNode.connect(graph1StereoPannerNode);
      ...
      vpd1Oscillator.frequency.setValueAtTime(0.000001, audioCtx.currentTime);
  ...
  useEffect(() => {
    if (play && incrementalData1.length > 0) {
      ...
      const vpd1Oscillator = vpd1OscillatorRef.current;
      ...
      vpd1Oscillator.frequency.setValueAtTime(rescaleFrequency(0, 2.5, 200, 2000, record?.vpd), audioCtx.currentTime);
    }
  ...
  const stopSound = () => {
    ...
    if (play) {
      ...
      vpd1Oscillator.stop(audioCtx.currentTime + 0.25);
  ...
  const variable4HideOnClick = () => {
    ...
    graph1Ref.current.toggleVariableVisibility('vpd', variable4Hide);
...
useEffect(() => {
  const updatedRecords = data1.map(record => {
  ...
  if (dataRange === '-1') {
    if (updatedRecords && updatedRecords.length > 0) setRefinedDataValues1([updatedRecords[updatedRecords.length - 1]]);
    else setRefinedDataValues1([]);
    return;
  }
  if (dataRange === '0') {
    setRefinedDataValues1(updatedRecords);
    return;
  }
  ...
  setRefinedDataValues1(updatedRecords.filter(item => {
...
useEffect(() => {
  ...
  if (dataRange === '-1') {
    if (updatedRecords && updatedRecords.length > 0) setRefinedDataValues2([updatedRecords[updatedRecords.length - 1]]);
  ...
  return (
    <PageLayout title='Derived Data Preset 2' backLink='derived-data' help helpOnClick={helpClick} dataControls treeOnClick={treeClick} aboutOnClick={aboutClick}>
      {
          loading1 || loading2
            ? <h4>Loading...</h4>
            : hide
